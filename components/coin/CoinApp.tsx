...
import {
  detailReflectionPrompts, guidanceForCategory, normalizeCoinCategory,
  type CoinCategory, type CoinLocale,
} from "@/lib/coinInterpretation";
import { categoryLabel, COIN_CATEGORIES, COIN_COPY } from "./coinCopy";
...
function readSession(): (Omit<CoinSessionV1, "category"> & { category: CoinCategory }) | null {
  try {
    ...
    return { ...parsed, category: normalizeCoinCategory(parsed.category) };
  } catch { return null; }
}

function clearStoredSession() { try { localStorage.removeItem(SESSION_KEY); } catch {} }

function LanguageSwitch({ locale, onSwitch }: { locale: CoinLocale; onSwitch: (locale: CoinLocale) => void }) {
  const copy = COIN_COPY[locale];
  const options: { locale: CoinLocale; label: string; ariaLabel: string }[] = locale === "ja"
    ? [{ locale: "ja", label: "日本語", ariaLabel: "日本語" }, { locale: "en", label: "EN", ariaLabel: "English" }]
    : [{ locale: "en", label: "EN", ariaLabel: "English" }, { locale: "ja", label: "日本語", ariaLabel: "Japanese" }];
  return <div className="coin-language" aria-label={copy.languageAria}>
    {options.map((option, index) => <span className="coin-language-option" key={option.locale}>
      {index > 0 && <span aria-hidden>/</span>}
      <button type="button" className={locale === option.locale ? "is-active" : ""} aria-label={option.ariaLabel} aria-current={locale === option.locale ? "page" : undefined} onClick={() => onSwitch(option.locale)}>{option.label}</button>
    </span>)}
  </div>;
}
...
function CoinTopbar({ locale, label, backLabel, onBack, onLanguageSwitch, muted, onMute, disabled = false }: {
  locale: CoinLocale; label: string; backLabel: string; onBack: () => void; onLanguageSwitch: (locale: CoinLocale) => void;
  muted: boolean; onMute: () => void; disabled?: boolean;
}) {
  return <div className="coin-topbar">
    <button className="coin-back" onClick={onBack} disabled={disabled}>← {backLabel}</button>
    <span>{label}</span>
    <div className="coin-topbar-tools"><LanguageSwitch locale={locale} onSwitch={onLanguageSwitch} /><IconButton locale={locale} muted={muted} onClick={onMute} inToolbar /></div>
  </div>;
}
...
export default function CoinApp({ locale = "ja", initialQuestion = "" }: { locale?: CoinLocale; initialQuestion?: string }) {
  const copy = COIN_COPY[locale];
  ...
  const categoryGuidance = primary ? guidanceForCategory(primary.number, category, locale) : "";
  const reflectionPrompts = primary ? detailReflectionPrompts(question, category, primary.number, relating?.number ?? null, locale) : [];
  ...
  const switchLanguage = (next: CoinLocale) => { if (next === locale) return; reset(); window.location.assign(next === "en" ? "/coin/en" : "/coin"); };
  ...
  const askAi = async () => {
    ...
    try {
      const response = await fetch("/api/iching/coin-interpret", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ locale, question, category, primaryNumber: primary.number, changingLineIndexes: reading.changingLineIndexes, relatingNumber: relating?.number ?? null }) });
  ...
  return <main className="coin-app" lang={locale}>
    {phase === "question" && <><LanguageSwitch locale={locale} onSwitch={switchLanguage} /><IconButton locale={locale} muted={muted} onClick={toggleMute} /></>}
    ...
    <AnimatePresence mode="wait">
      ...
      {phase === "casting" && <motion.section key="casting" className="coin-screen coin-casting" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <CoinTopbar locale={locale} label={copy.throwCount(casts.length + 1)} backLabel={copy.backQuestion} onBack={() => setPhase("question")} onLanguageSwitch={switchLanguage} muted={muted} onMute={toggleMute} disabled={rolling} /><h1 ref={screenTitleRef} tabIndex={-1}>{copy.title}</h1>
      ...
      {(phase === "result" || phase === "detail") && reading && primary && entry && text && <motion.section key={phase} className={`coin-screen coin-${phase}`} initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
        <CoinTopbar locale={locale} label={phase === "detail" ? copy.readFromHexagram : copy.result} backLabel={phase === "detail" ? copy.backResult : copy.backStart} onBack={() => phase === "detail" ? setPhase("result") : reset()} onLanguageSwitch={switchLanguage} muted={muted} onMute={toggleMute} /><h1 ref={screenTitleRef} tabIndex={-1}>{phase === "detail" ? copy.readFromHexagram : copy.title}</h1>
        {phase === "result" ? <>
