...
- `/` is the AWAI Commons brand page. Its main theme is「易の知恵を、暮らしに生かす」; it links to the five public products, briefly explains「いま易を学ぶ理由」, and shows up to two recent Note articles from the five-minute cached RSS feed. Do not restore public links to `/ask`, `/formal`, `/dice`, or `/card`.
...
- Open `/` at 320px, 390px, and 430px: the wheel is followed by `易から見えてくるもの`, the compact 3-perspective panel, then a centered `易を学ぶ 五つの入口` heading above the one-line flow `学ぶ → つくる → 占う → 読む` and 5 horizontal product rows, including「易のことば」at `/kotoba`. The flow-to-card gap must be 8px. After the products, confirm the living-wisdom concept card, fixed beginner link, up to two RSS article links, and Note profile link. Confirm there are no links to `/ask`, `/formal`, `/dice`, or `/card`. Open `/ask` directly and walk the flow: worry input → candidates → confirm → mode select → casting animation → interpretation sections → save → history re-read. Quick modes at `/formal` and `/dice` still play standalone with playback controls.
- Verify `/sitemap.xml` contains `/`, `/taikyoku`, `/hakke`, `/coin`, `/coin/en`, `/hexagrams`, `/hexagrams/1` through `/hexagrams/64`, `/kotoba`, and all five `/kotoba/[slug]` routes. Verify only the non-public core HTML pages emit `noindex, follow` while remaining directly accessible.
